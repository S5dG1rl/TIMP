...
QByteArray auth(QString log, QString pass)
{
    ...
    if (!query.exec()) {
        qCritical() << "Database error during authentication for user" << log 
                   << ":" << query.lastError().text();
        return "Error: Database error during authentication\n";
    }
    
    if (query.next()) {
        // Update last login time if needed
        QSqlQuery updateQuery(db);
        updateQuery.prepare("UPDATE users SET stat = stat + 1 WHERE login = :login");
        updateQuery.bindValue(":login", log);
        if (!updateQuery.exec()) {
            qWarning() << "Failed to update login stat for user" << log;
        }
        
        qDebug() << "User authenticated successfully:" << log;
        return "auth\n";
    } else {
        qDebug() << "Authentication failed for user:" << log;
...
QByteArray reg(QString log, QString pass, QString mail)
{
    ...
    checkQuery.bindValue(":email", mail);
    
    if (!checkQuery.exec()) {
        qCritical() << "Database error during user check:" << checkQuery.lastError().text();
    ...
    // Prepare the insert query with transaction
    db.transaction();
    
    QSqlQuery query(db);
    query.prepare("INSERT INTO users (login, password, email, sockid) "
                  "VALUES (:login, :password, :email, :sockid)");
    
    query.bindValue(":login", log);
    query.bindValue(":password", pass);
    query.bindValue(":email", mail);
    query.bindValue(":sockid", -1);  // Default sockid
    
    if (query.exec()) {
        db.commit();
        qDebug() << "User registered successfully:" << log;
        return "reg\n";
    } else {
        db.rollback();
        qCritical() << "Registration failed for user" << log << ":" << query.lastError().text();
        return "Error: Registration failed: " + query.lastError().text().toUtf8() + "\n";
    }
}
...
QByteArray task1(const QString &text)
{
    ...
    try {
        // Создаем экземпляр RSA с ключом 2048 бит
        RSAWrapper rsa(2048);
        
        // Шифруем текст
        std::string encryptedBase64 = rsa.encrypt(text.toStdString());
        
        // Для отладки: расшифровываем и выводим
        std::string decryptedText = rsa.decrypt(encryptedBase64);
        ...
        qDebug() << "RSA Error:" << e.what();
        return "Error: " + QByteArray(e.what());
    }
}
